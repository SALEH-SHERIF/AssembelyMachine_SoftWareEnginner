...
namespace AssemblerMachine
{
    public partial class Form1 : Form
    {
        ...
        private async void BtnAssemble_Click(object sender, EventArgs e)
        {
            ...
            bool isValid = true; // Tracks if all inputs are valid
            string errorMessage = ""; // Stores error message if validation fails

            // Variables to store Wheel and Motor numbers for later validation
            int wheelNumber = 0;
            int motorNumber = 0;
            ...
            foreach (var field in numberFields)
            {
                ...
                // Store Wheel and Motor numbers for later validation
                ...
                // If number > 0, ensure a type is selected
            ...
            // Ensure both Wheel and Motor numbers are greater than 0
            if (isValid)
            {
                if (wheelNumber == 0)
                {
            ...
            var car = new Car()
            {
                DoorType = cmbDoorType?.SelectedItem?.ToString() ?? "unselectedDoor",
                DoorNumber = int.Parse(txtDoorNumber.Text),
                GlassType = cmbGlassType?.SelectedItem?.ToString() ?? "unselectedGlass",
                GlassNumber = int.Parse(txtGlassNumber.Text),
                MotorType = cmbMotorPower.SelectedItem.ToString() ?? "unselectedMotor",
                MotorNumber = int.Parse(txtMotorNumber.Text),
                WheelNumber = int.Parse(txtWheelNumber.Text),
                WheelType = cmbWheelType?.SelectedItem?.ToString() ?? "unselectedWheel"
            };
            ...
            try
            {
                // Execute the assembly process asynchronously
            ...
            {
                // Hide progress bar and revert button text
